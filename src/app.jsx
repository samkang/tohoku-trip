...
const appId =
  typeof __app_id !== "undefined" ? __app_id : "tohoku-trip-v3-full";
...
const initialTripData = [
  {
    ...
    items: [
      ...
      },
      {
        ...
        story:
          "仙台牛舌的起源可以追溯到二戰後，由佐野啟四郎先生首創。善治郎堅持手工切肉與長時間熟成，是當地人排隊也要吃的名店。",
      },
      {
        id: "d1-4",
        type: "hotel",
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        type: "spot",
        time: "16:20",
        title: "銀山溫泉 Blue Hour",
        subTitle: "大正浪漫的魔幻時刻",
        location: "山形県尾花沢市銀山新畑",
        desc: "日落後天空呈現深藍色，煤氣燈亮起的瞬間，是銀山溫泉最美的時刻。",
        highlight: true,
        story:
          "銀山溫泉曾是江戶時代的銀礦區，後來沒落。直到大正時期挖掘出溫泉，並興建了洋風木造建築，才形成如今獨特的景觀。",
    ...
    items: [
      {
        ...
        highlight: false,
        story:
          "盛岡是一座充滿文學與歷史的城市，宮澤賢治與石川啄木都曾在此生活。市區內保留了許多明治時期的洋風建築。",
      },
      {
        id: "d4-2",
        type: "food",
        time: "12:30",
        title: "東家 本店 (Wanko Soba)",
        subTitle: "わんこそば",
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        highlight: true,
        story:
          "花卷是日本國民作家宮澤賢治的故鄉。這裡將他的作品如《銀河鐵道之夜》具象化，充滿了宇宙、自然與幻想的元素。",
      },
      {
        id: "d5-3",
        type: "food",
        time: "13:00",
        title: "山貓軒 (Wildcat House)",
        subTitle: "注文の多い料理店",
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-stone-900/60 backdrop-blur-sm p-0 sm:p-4 animate-in fade-in">
      <div className="bg-[#FAF9F6] w-full max-w-md h-[90vh] sm:h-auto sm:max-h-[85vh] rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden flex flex-col animate-in slide-in-from-bottom-10">
        <div className="h-32 bg-stone-200 relative flex items-center justify-center overflow-hidden">
          ...
          <button
            onClick={onClose}
            className="absolute top-4 right-4 bg-white/50 p-2 rounded-full backdrop-blur-md hover:bg-white transition-colors z-10"
          >
        ...
        <div className="flex-1 overflow-y-auto p-6 pt-0">
          ...
          {item.reservationNo && (
            <div className="mb-8 bg-amber-50 border border-amber-100 p-5 rounded-2xl relative overflow-hidden">
              ...
              <p className="text-amber-800/60 text-xs font-bold tracking-widest uppercase mb-1">
                RESERVATION
              </p>
              <p className="text-amber-900 text-lg font-serif font-bold">
                {item.title}
              </p>
              <div className="mt-3 bg-white/80 p-3 rounded-lg backdrop-blur-sm">
                <p className="text-xs text-stone-400 mb-1">Confirmation No.</p>
              ...
              <p className="text-xs text-amber-700 mt-2 flex items-center">
                <span className="mr-1">ℹ️</span> Check-in 時請出示此畫面
              </p>
            </div>
          )}
            ...
            <div className="mb-8">
              ...
              <div className="space-y-3">
                {item.menu.map((m, idx) => (
                  <div
                    key={idx}
                    className="bg-white border border-stone-100 p-4 rounded-xl shadow-sm"
                  >
                    <p className="text-xl font-bold text-stone-900 mb-1 font-serif">
                      {m.jp}
                    </p>
                    <div className="flex justify-between items-end">
                      <p className="text-stone-500 text-sm">{m.cn}</p>
                      {m.price && (
                        <p className="text-stone-800 font-mono text-sm bg-stone-100 px-2 py-0.5 rounded">
                          {m.price}
                        </p>
                      )}
                    </div>
          ...
          <div className="space-y-3">
            {expenses.length === 0 ? (
              ...
              expenses.map((ex) => (
                <div
                  key={ex.id}
                  className="bg-white p-4 rounded-xl shadow-sm border border-stone-100 flex justify-between items-center group"
                >
                  <div>
...
// ---------------------------------------------------------
// 5. Main App (Updated Tab Bar Layout)
// ---------------------------------------------------------
...
const TohokuTripApp = () => {
  ...
  // Weather & Auth Effects (Same as before)
  useEffect(() => {
    const fetchWeather = async () => {
      setIsWeatherLoading(true);
      try {
        const updatedData = await Promise.all(
          initialTripData.map(async (day) => {
            ...
            try {
              ...
              if (data.current_weather) {
                ...
                return {
                  ...day,
                  weather: { icon: icon, temp: `${temperature}°C`, desc: desc },
                };
              }
              return day;
            } catch (err) {
  ...
  return (
    <div className="bg-[#FAF9F6] min-h-screen font-sans text-stone-800 max-w-md mx-auto shadow-2xl relative overflow-hidden flex flex-col">
      ...
      <header className="pt-12 pb-6 px-6 bg-white relative border-b border-stone-100">
        <div className="flex justify-between items-start mb-6">
          <div>
            ...
            <div className="flex items-center mt-2 text-stone-500 text-xs font-medium">
              <span className="bg-stone-100 px-2 py-0.5 rounded text-stone-600 mr-2">
                2025
              </span>
          ...
          <div
            className="text-right cursor-pointer active:opacity-60 transition-opacity"
        ...
        <div className="flex gap-4 overflow-x-auto pb-2 no-scrollbar">
          {tripData.map((d, i) => (
            <div
              key={i}
              className="flex-shrink-0 flex flex-col items-center min-w-[60px]"
            >
          ...
          <div key={dIdx} className="mb-10">
            ...
            <div className="space-y-4 pl-2 relative">
              ...
              {day.items.map((item, iIdx) => (
                ...
                >
                  <div
                    className={`absolute left-0 top-1.5 w-3.5 h-3.5 rounded-full border-2 border-[#FAF9F6] shadow-sm z-10 
                    ${item.highlight ? "bg-amber-400" : "bg-stone-300"}`}
                  ></div>
                    ...
                    <div className="bg-white rounded-lg p-4 shadow-sm border border-stone-100 flex justify-between items-center">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className="text-[10px] px-1.5 py-0.5 rounded bg-stone-50 text-stone-500 uppercase tracking-wide">
                            {item.type}
                          </span>
                        </div>
      ...
      {/* --- BOTTOM NAVIGATION BAR (Replacing Floating Button) --- */}
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-stone-100 px-6 py-3 flex justify-around items-center z-40 max-w-md mx-auto safe-area-pb">
        ...
        {/* Center Add Button (Avoids CSB Overlay) */}
        <button
